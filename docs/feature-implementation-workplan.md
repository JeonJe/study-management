...
| 기능 | 호환 전략 |
| --- | --- |
| 운영 단위 관리 | 기존 `3기` 데이터는 `loop-pak-3` 운영 단위로 이관하고, 신규 쓰기는 운영 단위가 명시된 경로에서만 허용 |
...
- 운영 단위가 없는 기존 데이터는 마이그레이션 후 `loop-pak-3` 운영 단위로 정상 표시된다.
...
- 운영 단위 개념 추가
- 기존 `3기` 데이터를 `loop-pak-3` 운영 단위에 연결
...
1. 데이터베이스 구조 문서 최신화
2. 역할별 사용자 흐름 정리
3. 기능 스위치와 `loop-pak-3` 이관 단위 준비
4. 운영 단위 개념 추가
5. 기존 데이터 `loop-pak-3` 운영 단위 연결
6. 기존 화면 호환성 확인
7. 역할별 화면과 접근 범위 구현
8. 엔젤 주간 보고 구현
9. 모집 인원 관리 구현
