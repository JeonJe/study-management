...
4. 엔젤/관리자 화면은 역할별 접근 세션을 추가로 확인한다.
...
앱은 기수 입장, 전체관리자, 엔젤, 기수 관리자 접근을 분리해서 검증한다. 기수 입장 권한은 해당 기수의 일반 화면에만 사용하고, 전체관리자 권한은 `/admin`의 기수 관리에만 사용한다.

엔젤/관리자 역할 접근은 기수별 코드와 서버 측 세션 검증을 함께 사용한다. 관리자 역할은 엔젤 화면도 열 수 있지만, 엔젤 역할은 관리자 화면을 열 수 없다.

기수별 입장 코드, 엔젤 코드, 관리자 코드는 운영자가 현재값을 관리할 수 있도록 보호 저장한다. 구체적인 저장 방식, 세션 이름, 검증 로직은 공개 문서에 상세히 적지 않고 코드와 환경 변수 설정을 기준으로 관리한다.
